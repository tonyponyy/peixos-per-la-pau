...
function GeneraDesig(){
  var vull = ["Vull","M'agradaria","A poguer ser","Si us plau, vull","Seria feliç si hagués"]
  var desig=["un mar net.","mes menjar.","un aquari mes gran","mes temps per jugar.","un estiu etern."]
  desitg = vull[parseInt(Math.random()*vull.length)] +" "+desig[parseInt(Math.random()*desig.length)]
  return desitg;
}

function GeneraNom(){
  var noms = ["Nemo","Marta","Isaura","Pep","Martí","Jordi","Carles","Miranda","Juli"]
  return noms[parseInt(Math.random()*noms.length)]
}


function enviaPeix(){
  var missatge = ["Mirant la nova casa ...","Falta poquet...","Paciencia...","Climatitzant la peixera...","Netejant l'aigua...","Transferint el peix...","Netejant el vidre de la peixera..."]
  document.getElementById("enviant").innerHTML = missatge[parseInt(Math.random()*missatge.length)]
}
